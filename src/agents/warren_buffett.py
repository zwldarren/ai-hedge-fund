...
def analyze_moat(metrics: list) -> dict[str, any]:
    ...
    return {
        ...
        "details": "; ".join(reasoning)
        if reasoning
        else "Limited moat analysis available",
    }
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    return {
        ...
        "assumptions": {
            ...
            if "conservative_growth" in locals()
            else None,
        },
        "details": details,
    }
...
def analyze_book_value_growth(financial_line_items: list) -> dict[str, any]:
    ...
    if len(book_values) < 3:
        return {
            "score": 0,
            "details": "Insufficient book value data for growth analysis",
        }
    ...
    growth_periods = sum(
        1 for i in range(len(book_values) - 1) if book_values[i] > book_values[i + 1]
    )
...
def _calculate_book_value_cagr(book_values: list) -> tuple[int, str]:
    ...
    if oldest_bv > 0 and latest_bv > 0:
        ...
        return (
            0,
            "Unable to calculate meaningful book value CAGR due to negative values",
        )
